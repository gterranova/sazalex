...
export const uploadRoutes = uploadPath => {
  ...
  uploadRouter.use('', (req, res, next) => {
    ...
    form = Object.assign(form, {
      multiples: true,
      keepExtensions: true,
      uploadDir: uploadDir, // Set standard upload dir
      encoding: 'utf-8',
    ...
    form.on('fileBegin', (name, file) => {
      ...
      //console.log("Upload to", join(uploadDir, file.name))
      file.path = join(uploadDir, file.name);
    });
    ...
    form.on('field', (name, value) => {
      if (name === 'uploadPath') {
        uploadDir = form.uploadDir = join(uploadPath, value);
      }
    });
