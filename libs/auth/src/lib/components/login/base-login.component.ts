...
export abstract class BaseLoginComponent {
  ...
  constructor(
    ...
    this.user = new User();
    this.user.email = 'admin@sazalex.com';
  ...
  forgotPassword() {
    this.prompt({
      ...
      message: `Enter the email address you used to register for ${
        this.app_name
      ...
      if (data && data.result) {
        this.authService
          .userPasswordReset(data.text.trim())
          .toPromise()
          .then(() => {
  ...
  canSubmit() {
    return (
      !!this.user.email &&
      !!this.user.password &&
      (this.isLoggingIn || this.user.password === this.user.confirmPassword)
    );
  }
